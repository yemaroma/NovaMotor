<?php
	...
	class userController extends mainModel{
		...
		public function eliminarUsuarioControlador(){
			...
			if($eliminarUsuario->rowCount()==1){
				...
				$alerta=[
					...
					"texto"=>"El usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." se eliminó con exito",
					"icono"=>"success"
				];
			}else{
				$alerta=[
		...
		public function actualizarUsuarioControlador(){
			...
			if($this->actualizarDatos("usuario",$usuario_datos_up,$condicion)){
				...
				$alerta=[
					...
					"texto"=>"El usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." se actualizó con exito",
					"icono"=>"success"
				];
			}else{
		...
		# Controlador Actualizar Foto Usuarios #
		public function actualizarFotoUsuarioControlador(){
    		...
    		if($_FILES['usuario_foto']['name']=="" && $_FILES['usuario_foto']['size']<=0){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No ha seleccionado una foto valida para el usuario",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
			...
			if($this->actualizarDatos("usuario",$usuario_datos_up,$condicion)){
				...
				$alerta=[
					...
					"texto"=>"La foto del usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." se actualizó con exito",
					"icono"=>"success"
				];
			}else{
				...
				$alerta=[
					...
					"texto"=>"No hemos podido actualizar algunos datos del usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." sin embargo la foto se actualizó con exito",
					"icono"=>"warning"
				];
			}
			return json_encode($alerta);
		...
		# Controlador Eliminar Foto usuarios # 
		public function eliminarFotoUsuarioControlador(){
			...
			if(is_file($img_dir.$datos['usuario_foto'])){
				...
				if(!unlink($img_dir.$datos['usuario_foto'])){
					$alerta=[
						"tipo"=>"simple",
						"titulo"=>"Ocurrió un error inesperado",
						"texto"=>"Error al intentar eliminar la foto del usuario, por favor intenta nuevamente",
						"icono"=>"error"
					];
					return json_encode($alerta);
					exit();
			...
			if($this->actualizarDatos("usuario",$usuario_datos_up,$condicion)){
				...
				$alerta=[
					...
					"texto"=>"La foto del usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." se eliminó con exito",
					"icono"=>"success"
				];
			}else{
				...
				$alerta=[
					"tipo"=>"recargar",
					"titulo"=>"Foto eliminada",
					"texto"=>"No hemos podido actualizar algunos datos del usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." sin embargo la foto se eliminó con exito",
					"icono"=>"warning"
				];
			}
			return json_encode($alerta);
